...
const PROTECT_WL_FILE = path.join(DATA_DIR, "protect_whitelist.json");
...
const commandDefinitions = {
    ...
    giveaway: { category: "Configuration", label: "+giveaway", description: "Lancer un giveaway", allow: (message) => hasRoleCommandAccess(message, "giveaway") },
    reroll: { category: "Configuration", label: "+reroll", description: "Relancer un giveaway", allow: (message) => hasRoleCommandAccess(message, "reroll") },
    ...
    renew: { category: "Configuration", label: "+renew", description: "Recréer le salon identiquement", allow: (message) => hasRoleCommandAccess(message, "renew") },
    ...
    rename: { category: "Configuration", label: "+rename", description: "Renommer un salon ticket", allow: (message) => hasRoleCommandAccess(message, "rename") },
...
let antilinkSettings = loadJSON(ANTILINK_FILE);
let protectWhitelist = loadJSON(PROTECT_WL_FILE);
...
function saveAntilink() { saveJSON(ANTILINK_FILE, antilinkSettings); }
function saveProtectWhitelist() { saveJSON(PROTECT_WL_FILE, protectWhitelist); }

function isProtectWhitelisted(guildId, userId) {
    return userId === BOT_OWNER_ID || (Array.isArray(protectWhitelist[guildId]) && protectWhitelist[guildId].includes(userId));
}

function addProtectWhitelist(guildId, userId) {
    if (!Array.isArray(protectWhitelist[guildId])) protectWhitelist[guildId] = [];
    if (!protectWhitelist[guildId].includes(userId)) {
        protectWhitelist[guildId].push(userId);
...
function removeProtectWhitelist(guildId, userId) {
    if (!Array.isArray(protectWhitelist[guildId])) return false;
    const before = protectWhitelist[guildId].length;
    protectWhitelist[guildId] = protectWhitelist[guildId].filter(id => id !== userId);
    if (protectWhitelist[guildId].length !== before) {
        saveProtectWhitelist();
        return true;
    }
    return false;
...
const ANTILINK_EXEMPT_CATEGORY = "1489965799216255066";
const DISCORD_INVITE_REGEX_PROTECT = /(?:https?:\/\/)?(?:www\.)?(?:discord\.gg|discord\.com\/invite)\/\S+/i;
...
protectClient.on("messageCreate", async (message) => {
    ...
    // Anti-link: supprimer les invitations Discord si activé (sauf catégorie exemptée)
    if (antilinkSettings[message.guild.id]?.enabled) {
        if (
            message.channel?.parentId !== ANTILINK_EXEMPT_CATEGORY &&
            message.channel?.id !== ANTILINK_EXEMPT_CATEGORY &&
            DISCORD_INVITE_REGEX_PROTECT.test(message.content) &&
            message.author.id !== BOT_OWNER_ID &&
            !isProtectWhitelisted(message.guild.id, message.author.id)
        ) {
            const me = message.guild.members.me;
            if (me?.permissionsIn(message.channel)?.has(PermissionsBitField.Flags.ManageMessages)) {
                await message.delete().catch(() => {});
                message.channel.send(`${message.author}, les invitations Discord ne sont pas autorisées ici.`)
    ...
    // Réponse au ping avec le préfixe
    if (protectClient.user && message.mentions.has(protectClient.user) && !message.mentions.everyone) {
        ...
        if (onlyMention || startsWithMention) {
            message.reply(`Mon préfixe est **${PROTECT_PREFIX}**`).catch(() => {});
        }
    }
    ...
    if (command === "help") {
        const helpEmbed = new EmbedBuilder()
            ...
            .setDescription(`Préfixe : **${PROTECT_PREFIX}**`)
            .setColor("#ff0000")
            .addFields(
                { name: `${PROTECT_PREFIX}help`, value: "Afficher cette page d'aide", inline: false },
                { name: `${PROTECT_PREFIX}wl <@utilisateur>`, value: "Ajouter un utilisateur à la whitelist protect *(owner uniquement)*", inline: false },
                { name: `${PROTECT_PREFIX}unwl <@utilisateur>`, value: "Retirer un utilisateur de la whitelist protect *(owner uniquement)*", inline: false },
                { name: `${PROTECT_PREFIX}anti link`, value: "Activer/désactiver la suppression des liens d'invitation Discord", inline: false }
            )
            .setFooter({ text: "Bot Protect" });
        return message.channel.send({ embeds: [helpEmbed] });
    }

    // <wl @user (owner uniquement)
    if (command === "wl") {
        if (message.author.id !== BOT_OWNER_ID) {
            return message.reply("Seul le propriétaire du bot peut utiliser cette commande.");
        }
        const targetId = getIdFromMentionOrId(args[0]);
        if (!targetId) return message.reply(`Utilisation : ${PROTECT_PREFIX}wl <@utilisateur ou ID>`);

        const added = addProtectWhitelist(message.guild.id, targetId);
        if (added) {
            return message.reply(`<@${targetId}> a été ajouté à la whitelist protect.`);
        }
        return message.reply(`<@${targetId}> est déjà dans la whitelist protect.`);
    }

    // <unwl @user (owner uniquement)
    if (command === "unwl") {
        if (message.author.id !== BOT_OWNER_ID) {
            return message.reply("Seul le propriétaire du bot peut utiliser cette commande.");
        }
        const targetId = getIdFromMentionOrId(args[0]);
        if (!targetId) return message.reply(`Utilisation : ${PROTECT_PREFIX}unwl <@utilisateur ou ID>`);

        const removed = removeProtectWhitelist(message.guild.id, targetId);
        if (removed) {
            return message.reply(`<@${targetId}> a été retiré de la whitelist protect.`);
        }
        return message.reply(`<@${targetId}> n'est pas dans la whitelist protect.`);
    }

    // <anti link
    if (command === "anti" && args[0]?.toLowerCase() === "link") {
        if (message.author.id !== BOT_OWNER_ID) {
            return message.reply("Seul le propriétaire du bot peut utiliser cette commande.");
        }
        const guildId = message.guild.id;
        if (!antilinkSettings[guildId]) antilinkSettings[guildId] = {};

        antilinkSettings[guildId].enabled = !antilinkSettings[guildId].enabled;
        saveAntilink();

        const status = antilinkSettings[guildId].enabled ? "activé" : "désactivé";
        const embed = new EmbedBuilder()
            .setTitle("Anti-Link")
            .setDescription(`Le système anti-link a été **${status}**.`)
            .addFields(
                { name: "Catégorie exemptée", value: `<#${ANTILINK_EXEMPT_CATEGORY}>`, inline: false }
            )
            .setColor(antilinkSettings[guildId].enabled ? "#00ff00" : "#ff0000");
