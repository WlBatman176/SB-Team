...
const PROTECT_WHITELIST_FILE = path.join(DATA_DIR, "protect_whitelist.json");
const PROTECT_CONFIG_FILE = path.join(DATA_DIR, "protect_config.json");
...
let protectWhitelist = loadJSON(PROTECT_WHITELIST_FILE);
let protectConfig = loadJSON(PROTECT_CONFIG_FILE);
...
function saveProtectWhitelist() { saveJSON(PROTECT_WHITELIST_FILE, protectWhitelist); }
function saveProtectConfig() { saveJSON(PROTECT_CONFIG_FILE, protectConfig); }
...
function isProtectWhitelisted(guildId, userId) {
    return userId === BOT_OWNER_ID || (protectWhitelist[guildId] && protectWhitelist[guildId].includes(userId));
}

function getProtectConfig(guildId) {
    if (!protectConfig[guildId]) protectConfig[guildId] = { antiLink: false, antiRole: false, antiRaid: false };
    return protectConfig[guildId];
}
...
// ===================== BOT PROTECT - COMMANDES (prefix <) =====================

const DISCORD_LINK_REGEX = /(?:https?:\/\/)?(?:www\.)?(?:discord\.gg|discord\.com\/invite)\/\S+/i;
const GENERAL_LINK_REGEX = /https?:\/\/\S+/i;

const protectCommandDefinitions = {
    help: { label: "<help", description: "Voir les commandes du bot protect" },
    "anti": { label: "<anti link / <anti role / <anti raid", description: "Activer/désactiver les protections" },
    wl: { label: "<wl", description: "Ajouter un utilisateur à la whitelist protect (owner)" },
    unwl: { label: "<unwl", description: "Retirer un utilisateur de la whitelist protect (owner)" }
};

const roleCreationTracker = new Map();

protectClient.on("messageCreate", async (message) => {
    if (message.author.bot) return;

    if (protectClient.user && message.mentions.has(protectClient.user) && !message.mentions.everyone) {
        const onlyMention = /^<@!?\d+>\s*$/.test(message.content.trim());
        const startsWithMention = new RegExp(`^<@!?${protectClient.user.id}>`).test(message.content.trim());
        if (onlyMention || startsWithMention) {
            message.reply(`mon prefix est **${PROTECT_PREFIX}**`).catch(() => {});
        }
    }

    if (!message.content.startsWith(PROTECT_PREFIX)) return;

    const args = message.content.slice(PROTECT_PREFIX.length).trim().split(/ +/);
    const command = args.shift().toLowerCase();

    if (command === "help") {
        if (!isProtectWhitelisted(message.guild.id, message.author.id)) {
            return message.reply("tu n'es pas autorisé à utiliser cette commande");
        }

        const cfg = getProtectConfig(message.guild.id);
        ...
        const cmds = [
            `\`<help\` — Voir les commandes du bot protect`,
            `\`<anti link\` — Activer/désactiver l'anti lien (actuellement: ${cfg.antiLink ? "**ON**" : "**OFF**"})`,
            `\`<anti role\` — Activer/désactiver l'anti rôle - limite à 2 créations (actuellement: ${cfg.antiRole ? "**ON**" : "**OFF**"})`,
            `\`<anti raid\` — Activer/désactiver l'anti raid (actuellement: ${cfg.antiRaid ? "**ON**" : "**OFF**"})`
        ];
        helpEmbed.addFields({ name: "Commandes", value: cmds.join("\n"), inline: false });

        if (message.author.id === BOT_OWNER_ID) {
            const ownerCmds = [
                `\`<wl\` — Ajouter un utilisateur à la whitelist protect`,
                `\`<unwl\` — Retirer un utilisateur de la whitelist protect`
            ];
    ...
    if (command === "wl") {
        ...
        protectWhitelist[guildId] = protectWhitelist[guildId] || [];
        if (!protectWhitelist[guildId].includes(user.id)) {
            protectWhitelist[guildId].push(user.id);
            saveProtectWhitelist();
        }
        return message.reply(`${user.user.tag} ajouté à la whitelist protect`);
    }
    ...
    if (command === "unwl") {
        ...
        const guildId = message.guild.id;
        protectWhitelist[guildId] = protectWhitelist[guildId] || [];
        if (!protectWhitelist[guildId].includes(userId)) {
            return message.reply(`${userId} n'est pas en whitelist protect`);
        }
        const user = await message.guild.members.fetch(userId).catch(() => null);
        protectWhitelist[guildId] = protectWhitelist[guildId].filter(id => id !== userId);
        saveProtectWhitelist();
        return message.reply(`${(user ? user.user.tag : userId)} a été retiré de la whitelist protect`);
    }
    ...
    if (command === "anti") {
        ...
        const guildId = message.guild.id;
        const cfg = getProtectConfig(guildId);

        if (subCommand === "link") {
            cfg.antiLink = !cfg.antiLink;
            saveProtectConfig();
            return message.reply(`Anti link ${cfg.antiLink ? "**activé**" : "**désactivé**"}`);
        }

        if (subCommand === "role") {
            cfg.antiRole = !cfg.antiRole;
            saveProtectConfig();
            return message.reply(`Anti role ${cfg.antiRole ? "**activé** (limite: 2 rôles créés par membre/minute)" : "**désactivé**"}`);
        }

        if (subCommand === "raid") {
            cfg.antiRaid = !cfg.antiRaid;
            saveProtectConfig();
            return message.reply(`Anti raid ${cfg.antiRaid ? "**activé**" : "**désactivé**"}`);
        }

        return message.reply("Utilisation: `<anti link`, `<anti role` ou `<anti raid`");
    }
});

// Anti link — supprime les liens si activé (protect bot)
protectClient.on("messageCreate", async (message) => {
    if (message.author.bot) return;
    ...
    if (isProtectWhitelisted(message.guild.id, message.author.id)) return;

    if (GENERAL_LINK_REGEX.test(message.content)) {
        const me = message.guild.members.me;
        if (me?.permissionsIn(message.channel)?.has(PermissionsBitField.Flags.ManageMessages)) {
            await message.delete().catch(() => {});
            message.channel.send(`${message.author}, les liens ne sont pas autorisés ici`)
                .then(m => setTimeout(() => m.delete().catch(() => {}), 5000)).catch(() => {});
        }
    }
});

// Anti role — limite la création de rôles à 2 par membre (protect bot)
protectClient.on("roleCreate", async (role) => {
    const guild = role.guild;
    if (!guild) return;

    const cfg = getProtectConfig(guild.id);
    if (!cfg.antiRole) return;
    ...
    if (isProtectWhitelisted(guild.id, executor.id)) return;

    const key = `${guild.id}-${executor.id}`;
    const now = Date.now();
    const tracker = roleCreationTracker.get(key) || [];
    const recent = tracker.filter(t => now - t < 60000);
    recent.push(now);
    roleCreationTracker.set(key, recent);

    if (recent.length > 2) {
        await role.delete("Anti role: limite de création dépassée").catch(() => {});
        ...
        if (member) {
            ...
            await member.roles.remove(rolesToRemove, "Anti role: création de rôles abusive").catch(() => {});
        }

        const logsChannel = guild.channels.cache.find(c => c.name === "logs-protection");
        if (logsChannel) {
            logsChannel.send({ embeds: [new EmbedBuilder()
                .setTitle("Anti Role: création bloquée")
                .setDescription(`${executor.tag} (${executor.id}) a dépassé la limite de 2 rôles créés`)
                .setColor("#ff0000")
            ]}).catch(() => {});
        }
    }
...
// Anti raid — détecte les joins massifs (protect bot)
const joinTracker = new Map();

protectClient.on("guildMemberAdd", async (member) => {
    ...
    if (recentJoins.length >= 5) {
        ...
        if (logsChannel) {
            logsChannel.send({ embeds: [new EmbedBuilder()
                .setTitle("Anti Raid: raid détecté")
        ...
        try {
            ...
            if (logsChannel) {
                logsChannel.send("Anti Raid: niveau de vérification mis au **maximum**").catch(() => {});
            }
            setTimeout(async () => {
                try {
                    await guild.setVerificationLevel(1);
                    if (logsChannel) {
                        logsChannel.send("Anti Raid: niveau de vérification remis à **faible** après 60 secondes").catch(() => {});
                    }
                } catch (e) {
                    console.error("[Anti Raid] Erreur reset vérification:", e.message);
                }
            }, 60000);
        } catch (e) {
            console.error("[Anti Raid] Erreur set vérification:", e.message);
        }

        const recentMembers = guild.members.cache
            .filter(m => m.joinedTimestamp && now - m.joinedTimestamp < 15000 && !m.user.bot)
            .map(m => m);

        for (const m of recentMembers) {
            if (!isProtectWhitelisted(guildId, m.id)) {
                await m.kick("Anti Raid: suspicion de raid").catch(() => {});
            }
        }
    }
});
